...
async function findPlugins(dir, options) {
  console.log("findPlugins", dir, options);
...
function findCurrentDirHooks(
  ...
  console.log("findCurrentDirHooks");
...
function processIndexDirEntryIfExists(
  ...
  console.log("processIndexDirEntryIfExists");
  // Contains index file?
...
async function processDirContents(
  ...
  console.log("processDirContents");
...
function handleTypeScriptSupport(file, language, isHook = false) {
  console.log("handleTypeScriptSupport");
  if (
    language === "typescript" &&
